...
<link rel="stylesheet" href="/assets/css/style.css?v=20260328d">
<script src="/assets/js/animations.js?v=20260328d" defer></script>
</head><body>
...
def nav_html(active, lang, pfx, page_slug=""):
    ...
    opts="".join([f'<a class="lang-dd-item" href="{"/" + l + slug_clean + "/" if l!="en" else slug_clean+"/"}" hreflang="{LANG_LOCALE[l]}">{flag(l,18)} {LANG_NAMES[l]}</a>' for l in LANGS if l!=lang])
...
def footer_html(lang, pfx):
    ...
    return f'<footer><div class="container"><div class="footer-grid"><div><img src="{LOGO}" alt="Ngor Surfcamp Teranga" class="footer-brand-logo" loading="lazy"><p>{ABOUT[lang]}</p><div class="footer-social"><a href="https://wa.me/221789257025" target="_blank" class="soc-btn wa" aria-label="WhatsApp"><span style="display:inline-flex">{WA_ICO}</span></a><a href="https://www.instagram.com/ngorsurfcampteranga" target="_blank" class="soc-btn ig" aria-label="Instagram"><span style="display:inline-flex">{IG}</span></a><a href="https://www.tiktok.com/@ngorsurfcampteranga" target="_blank" class="soc-btn tt" aria-label="TikTok"><span style="display:inline-flex">{TT}</span></a></div></div><div class="footer-col"><h4>{EXP[lang]}</h4>{lk}</div><div class="footer-col"><h4>{"Contact" if lang in ["en","fr","es"] else "Contatti" if lang=="it" else "Kontakt"}</h4><a href="https://wa.me/221789257025" target="_blank">WhatsApp: +221 78 925 70 25</a><a href="mailto:info@surfcampsenegal.com">info@surfcampsenegal.com</a></div><div class="footer-col"><h4>{"Follow" if lang=="en" else "Suivez-nous" if lang=="fr" else "Síguenos" if lang=="es" else "Seguici" if lang=="it" else "Folgen"}</h4><a href="https://www.instagram.com/ngorsurfcampteranga" target="_blank">Instagram</a><a href="https://www.tiktok.com/@ngorsurfcampteranga" target="_blank">TikTok</a></div></div><div class="footer-bottom"><p>{COPY[lang]} &nbsp;·&nbsp; <a href="{pp_href}" class="footer-pp-link">{PP_LBL[lang]}</a></p><div class="footer-flags">{fl}</div></div></div></footer><a href="https://wa.me/221789257025" target="_blank" rel="noopener" id="float-wa" aria-label="WhatsApp"><span style="display:inline-flex">{WA_ICO}</span></a>'
...
def build_article(en_art, lang):
    ...
    img = f"/assets/images/{en_slug}.webp" if os.path.exists(f"{DEMO_DIR}/assets/images/{en_slug}.webp") else f"{WIX}/df99f9_961b0768e713457f93025f4ce6fb1419~mv2.jpg"
    ...
    def rel_card(r):
        ...
        r_img_src  = f"/assets/images/{r_slug}.webp" if os.path.exists(r_img_path) else f"{WIX}/df99f9_961b0768e713457f93025f4ce6fb1419~mv2.jpg"
        return (
...
FALLBACK_IMG = f"{WIX}/df99f9_961b0768e713457f93025f4ce6fb1419~mv2.jpg"
...
def build_blog_index(lang):
    ...
    BLOG_ICON = ico("stat-waves", 44)
    html += f"""
<main>
  <header class="blog-hub-header" role="banner">
    <div class="container" style="text-align:center;padding:56px 24px 68px">
      <div style="margin-bottom:18px;display:inline-flex;align-items:center;justify-content:center;width:72px;height:72px;border-radius:18px;background:rgba(240,192,96,0.18);border:1px solid rgba(240,192,96,0.25)">{BLOG_ICON}</div>
...
{{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
  ...
  {{"@type":"ListItem","position":3,"name":"{c_name}","item":"{SITE_URL}{c_href}"}}
    ...
    html = html.replace("</head>", crumb_ld + "\n</head>", 1)
    fq_block = get_footer_quotes(lang)
    html += nav_html("blog", lang, pfx, f"/blog/{CAT_SLUG_WORD[lang]}/{c_slug}")
    c_ico_name  = cdata.get("ico_name", "icon-surf-guide")
