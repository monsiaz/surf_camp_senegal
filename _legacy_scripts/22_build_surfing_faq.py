...
IMGS = {
    ...
    "coach":  f"{WIX}/df99f9_9e239b5550f949e998b00f8e16417fa8~mv2.jpg",
    "island": f"{WIX}/df99f9_56b9af6efe2841eea44109b3b08b7da1~mv2.jpg",
    "ngor_r": f"{WIX}/11062b_7f89d2db0ace4027ac4a00928a6aca08~mv2.jpg",
    "airport":f"{WIX}/df99f9_d6e404dd3cf74396b6ea874cb7021a27~mv2.jpg",
    "boat":   f"{WIX}/df99f9_4edccd3c01ca4f8ba06ed25c80e1eccb~mv2.jpg",
    "ngor_island_map": f"{WIX}/df99f9_d81668a18a9d49d1b5ebb0ea3a0abbc7~mv2.jpg",
}
...
def footer_html(lang, pfx):
    ...
    LINKS=[("/surf-house",{"en":"Surf House","fr":"Surf House","es":"Surf House","it":"Surf House","de":"Surf House","nl":"Surf House","ar":"بيت الأمواج"}),("/island",{"en":"Island","fr":"Île","es":"Isla","it":"Isola","de":"Insel","nl":"Eiland","ar":"الجزيرة"}),("/surfing",{"en":"Surfing","fr":"Surf","es":"Surf","it":"Surf","de":"Surfen","nl":"Surfen","ar":"ركوب الأمواج"}),("/blog",{"en":"Blog","fr":"Blog","es":"Blog","it":"Blog","de":"Blog","nl":"Blog","ar":"المدونة"}),("/gallery",{"en":"Gallery","fr":"Galerie","es":"Galería","it":"Galleria","de":"Galerie","nl":"Galerij","ar":"معرض الصور"}),("/faq",{"en":"FAQ","fr":"FAQ","es":"FAQ","it":"FAQ","de":"FAQ"}),("/booking",{"en":"Book Now","fr":"Réserver","es":"Reservar","it":"Prenota","de":"Buchen","nl":"Boeken","ar":"احجز الآن"})]
...
<main>
  <header class="page-header" style="background-image:url('{IMGS['island']}')" role="banner">
    <h1>{h1}</h1>
    <p>{"Complete guide to Ngor Surfcamp Teranga: getting there, surf levels, accommodation, waves and booking." if lang=="en" else "Guide complet sur le Ngor Surfcamp Teranga : comment s'y rendre, niveaux surf, hébergement, vagues et réservation." if lang=="fr" else "Guía completa del Ngor Surfcamp Teranga: cómo llegar, niveles surf, alojamiento, olas y reserva." if lang=="es" else "Guida completa al Ngor Surfcamp Teranga: come arrivare, livelli surf, alloggio, onde e prenotazione." if lang=="it" else "Vollständiger Leitfaden zum Ngor Surfcamp Teranga: Anreise, Surf-Level, Unterkunft, Wellen und Buchung."}</p>
  </header>

  <section class="section">
    <div class="container">
      <div style="display:grid;grid-template-columns:260px 1fr;gap:56px;align-items:flex-start" class="faq-layout">

        <!-- TOC Sidebar -->
        <aside style="position:sticky;top:88px;background:#f9fafb;border-radius:16px;padding:20px;border:1px solid #e5e7eb" class="faq-sidebar">
          <div style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:0.14em;color:var(--fire);margin-bottom:14px">{QUICK_ANS[lang]}</div>
...
<style>
@media(max-width:1024px){{.faq-layout{{grid-template-columns:1fr;gap:32px;}}.faq-sidebar{{position:relative;top:auto;}}}}
</style>"""
    html += footer_html(lang, pfx)
    html += close()
    return html
...
for lang in LANGS:
    ...
    write(f"{spfx}/surfing/", build_surfing_v2(lang))
    write(f"{spfx}/faq/", build_super_faq(lang))
    print(f"  ✅ {lang}: surfing + faq")

print(f"\nTotal pages: {total}")
print("✅ Surfing v2 + Super FAQ complete!")
