...
LANGS           = ["en","fr","es","it","de","nl","ar"]   # langs with full translations
SWITCHER_LANGS  = LANGS + ["pt","da"]                   # all langs shown in the switcher
...
def lang_dd(cur, page_slug):
    ...
    opts = "".join([f'<a class="lang-dd-item" href="{"/" + l + s + "/" if l!="en" else s+"/"}" hreflang="{LANG_LOCALE[l]}">{flag(l,18)} {LANG_NAMES[l]}</a>' for l in SWITCHER_LANGS if l!=cur])
...
def nav_html(active, lang, pfx, page_slug=""):
    ...
    return f'<nav id="nav"><div class="nav-inner"><a href="{pfx}/" class="nav-logo"><img src="{LOGO}" alt="Ngor Surfcamp Teranga" width="130" height="44" loading="eager"></a><div class="nav-links" id="nav-links">{items}</div><div class="nav-right">{lang_dd(lang,page_slug)}<a href="https://wa.me/221789257025" target="_blank" class="nav-wa" aria-label="WhatsApp"><span style="width:18px;height:18px;display:inline-flex">{WA_ICO}</span><span class="nav-wa-label">WhatsApp</span></a><button type="button" class="nav-toggle" id="nav-toggle" aria-label="Menu" aria-expanded="false" aria-controls="nav-links"><span style="width:22px;height:22px;display:inline-flex;color:#fff">{MENU_ICO}</span></button></div></div></nav>'
