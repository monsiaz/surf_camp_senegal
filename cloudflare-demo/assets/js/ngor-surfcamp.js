...
function initHeader(){const nav=$('#nav');if(!nav)return;let _scrolled=false;const update=()=>{const y=scrollY;const want=_scrolled?y>30:y>50;if(want!==_scrolled){_scrolled=want;nav.classList.toggle('scrolled',_scrolled);}};window.addEventListener('scroll',update,{passive:true});update();const toggle=document.getElementById('nav-toggle');const links=document.getElementById('nav-links');if(toggle&&links){const _closeMenu=()=>{links.classList.remove('open');toggle.classList.remove('active');toggle.setAttribute('aria-expanded','false');document.body.style.overflow='';};toggle.addEventListener('click',function(e){e.stopPropagation();const isOpen=links.classList.toggle('open');toggle.classList.toggle('active',isOpen);toggle.setAttribute('aria-expanded',isOpen?'true':'false');document.body.style.overflow=isOpen?'hidden':'';});document.addEventListener('click',function(e){if(!links.classList.contains('open'))return;const outsideMenu=!links.contains(e.target)&&e.target!==toggle&&!toggle.contains(e.target);const onBackdrop=e.target===links;if(outsideMenu||onBackdrop)_closeMenu();});document.addEventListener('keydown',function(e){if(e.key==='Escape'&&links.classList.contains('open'))_closeMenu();});}}
function initHeroText(){const h1=$('.hero-h1');if(!h1)return;$$('[data-lang]',h1).forEach(span=>{const words=span.textContent.split(' ');span.innerHTML=words.map((w,i)=>`<span class="word-reveal"style="animation-delay:${0.05 + i * 0.04}s">${w}</span>`).join(' ');});requestAnimationFrame(()=>h1.classList.add('words-split'));}
function initReveal(){const obs=new IntersectionObserver((entries)=>{entries.forEach(entry=>{if(entry.isIntersecting){const el=entry.target;const delay=el.dataset.delay||'0';el.style.transitionDelay=delay+'ms';el.classList.add('up');obs.unobserve(el);}});},{threshold:0.05,rootMargin:'0px 0px -20px 0px'});$$('.grid-3, .grid-2, .feat, .related-grid, .blog-grid').forEach(container=>{$$(':scope > *, :scope > .card',container).forEach((child,i)=>{child.dataset.delay=(i*80).toString();});});$$('.gallery-masonry').forEach(container=>{$$(':scope > .gallery-item',container).forEach((child,i)=>{child.dataset.delay=(i*55).toString();});});$$('.reveal').forEach(el=>{const r=el.getBoundingClientRect();if(r.top<innerHeight&&r.bottom>0){el.style.transitionDelay='0ms';el.classList.add('up');}else{obs.observe(el);}});}
function initFaqPageAccordion(){$$('.faq-q').forEach((btn)=>{btn.addEventListener('click',()=>{const item=btn.closest('.faq-item');if(!item)return;const wasOpen=item.classList.contains('open');const block=item.closest('.faq-section,.faq-accordion')||item.parentElement;block?.querySelectorAll('.faq-item.open').forEach((o)=>{if(o!==item){o.classList.remove('open');const ob=o.querySelector('.faq-q');if(ob)ob.setAttribute('aria-expanded','false');}});item.classList.toggle('open',!wasOpen);btn.setAttribute('aria-expanded',String(!wasOpen));});});}
...
document.addEventListener('DOMContentLoaded',()=>{function _safe(n,fn){try{fn()}catch(err){console.warn('[ngor-surfcamp] '+n,err)}}initHeader();_safe('initScrollProgress',initScrollProgress);_safe('initArticleToc',initArticleToc);_safe('initHeroText',initHeroText);_safe('initHeroVideoLoop',initHeroVideoLoop);_safe('initTrustStatsMotion',initTrustStatsMotion);_safe('initGhTeaserMap',initGhTeaserMap);_safe('initGalleryLightbox',initGalleryLightbox);_safe('initGalleryImgFade',initGalleryImgFade);_safe('initReveal',initReveal);_safe('initFaqPageAccordion',initFaqPageAccordion);_safe('initProseHashLinks',initProseHashLinks);scrollToHashTarget()});window.addEventListener('hashchange',scrollToHashTarget);window.toggleMenu=function(){
  var links=document.getElementById('nav-links');
  var btn=document.getElementById('nav-toggle');
  if(!links)return;
  var isOpen=links.classList.toggle('open');
  if(btn){btn.classList.toggle('active',isOpen);btn.setAttribute('aria-expanded',isOpen?'true':'false');}
...
(function(){const frame=document.querySelector('.hg-frame');if(!frame)return;let _raf=null;function applyParallax(){const r=frame.getBoundingClientRect();if(r.bottom<0||r.top>window.innerHeight){_raf=null;return;}const p=(window.innerHeight-r.top)/(window.innerHeight+r.height);const off=Math.round((p-0.5)*54*10)/10;frame.style.transform='translateY('+off+'px)';_raf=null;}window.addEventListener('scroll',()=>{if(!_raf)_raf=requestAnimationFrame(applyParallax);},{passive:true});applyParallax();})();
