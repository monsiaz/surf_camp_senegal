...
setInterval(()=>show(cur+1),3800);});(function initHomeGallery(){const viewport=document.getElementById('hg-viewport');const prevBtn=document.getElementById('hg-prev');const nextBtn=document.getElementById('hg-next');const dotsEl=document.getElementById('hg-dots');const progFill=document.getElementById('hg-progress-fill');const curNumEl=document.getElementById('hg-cur-num');if(!viewport)return;const imgs=viewport.querySelectorAll('.hg-img');const total=imgs.length;if(!total)return;const INTERVAL=3000;let cur=0;let timer=null;let paused=false;const dots=[];if(dotsEl){imgs.forEach((_,i)=>{const d=document.createElement('button');d.className='hg-dot'+(i===0?' hg-dot-active':'');d.setAttribute('aria-label','Photo '+(i+1));d.addEventListener('click',()=>{stopAuto();goTo(i);startAuto();});dotsEl.appendChild(d);dots.push(d);});}
function pad(n){return String(n+1).padStart(2,'0');}
function updateUI(idx){imgs.forEach((img,i)=>img.classList.toggle('hg-active',i===idx));dots.forEach((d,i)=>d.classList.toggle('hg-dot-active',i===idx));if(curNumEl)curNumEl.textContent=pad(idx);}
...
if(prevBtn)prevBtn.addEventListener('click',()=>{stopAuto();goTo(cur-1);startAuto();});if(nextBtn)nextBtn.addEventListener('click',()=>{stopAuto();goTo(cur+1);startAuto();});viewport.addEventListener('mouseenter',()=>{paused=true;});viewport.addEventListener('mouseleave',()=>{paused=false;});let touchX0=null;viewport.addEventListener('touchstart',e=>{touchX0=e.touches[0].clientX;},{passive:true});viewport.addEventListener('touchend',e=>{if(touchX0===null)return;const dx=e.changedTouches[0].clientX-touchX0;touchX0=null;if(Math.abs(dx)<30)return;stopAuto();goTo(dx<0?cur+1:cur-1);startAuto();});goTo(0);startAuto();})();(function initReviewsSlider(){function gapForTrack(track){const g=parseFloat(getComputedStyle(track).gap);return Number.isFinite(g)?g:16;}
